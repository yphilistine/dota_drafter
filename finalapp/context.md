...
GSI-таймаут: если нет обновлений > GSI_WATCHDOG_SEC (60с) - сброс на IDLE (Dota 2 закрыта).
...
| `SharedPortraitState` | 10 слотов портретов + две пары manual/detected (`manualPos`/`detectedPos` для позиций, `manualHero`/`detectedHero` для героев) + флаг active. `manualHero[i]`: 0 = авто, >0 = героя выбрали вручную в GUI, -1 = слот вручную объявлен пустым; ручной выбор побеждает распознавание во всех фазах. `detectedHero[i]` капчур пишет всегда, даже по перекрытому слоту - GUI показывает его первой строкой popup'а смены героя |
...
| `canonicalHeroKey(raw)` | Каноническая форма имени героя: нижний регистр, только буквы/цифры, без префикса `npc_dota_hero_` ("npc_dota_hero_crystal_maiden" и "Crystal Maiden" -> "crystalmaiden"). Общая для сопоставления портретов (`portrait_runner.cpp`, ключ по `heroes.name` - он стабилен, в отличие от `localized_name`) и лексического поиска героя в GUI (`gui_draw.cpp`) |
...
| `updateSpectatorState(body)` | Наблюдаемая игра (спектейт): `team_name`/`team_slot` каждого из 10 игроков - из вложенных `player.team2/team3.playerN`-блоков (границы блока - до следующего `playerN+1`, для player9 - до `previously`, тем же линейным поиском, что и `json_get`), слот в `g_spectatorState` вычисляется по ним, не по порядку playerN. Id героя - из ОТДЕЛЬНОГО top-level блока `"hero"` с той же вложенностью `team2/team3.playerN` (`hero.teamN.playerN.id`, не `player.teamN.playerN` - там id героя вообще нет, только экономика/бои); т.к. ключи `playerN` буквально совпадают в обоих top-level блоках, поиск под hero-id стартует не с начала body, а с позиции `"hero"` (`body.find(key, heroSectionPos)`), иначе `find()` попадал бы в первое (player-блок) совпадение и ничего не находил. Пишет `g_spectatorState.active=true` + `lastUpdate` |
...
| `runPortraitCapture(gameInfo, dbPath, running, out)` | Цикл захвата каждые 250мс -> `HeroRecognizer` (Pearson) для героев + `PosOcrRecognizer` (Windows OCR) для позиций -> запись в livepicks. Тело цикла обёрнуто в try/catch на итерацию (логирует и продолжает, не убивая поток - раньше во всей функции не было ни одного try/catch). На ветке ошибки открытия БД теперь вызывается `winrt::uninit_apartment()` (раньше пропускался, в отличие от нормального пути через `cleanup:`). Диагностика - через `LOG_INFO/WARN/ERR` (раньше `printf`/`puts`, невидимые в GUI-приложении без консоли). Без гистерезиса/фиксации: каждый кадр - всегда ближайший по Пирсону кандидат среди героев и `"null"` (own запись в базе хешей, участвует в том же argmax) немедленно перезаписывает слот, если отличается от уже записанного. Порог `score >= 0.4` - не решающий фильтр (решение - argmax по всей базе), а только отсев нечитаемых кадров. Расчёт: единичный кадр с ложным совпадением (напр. чат/scoreboard/шоп на секунду перекрыли регион) даёт максимум один неверный кадр в БД - следующий цикл (250мс) его перезапишет, так как ничего не "запоминается" против нового результата. Единственное исключение из "всегда верим кадру" - ручной выбор героя в GUI (`out.manualHero[slot]`, тот же приоритет, что у `manualPos` над OCR): такой слот распознаванием не перезаписывается, но результат кадра всё равно кладётся в `out.detectedHero[slot]` - его показывает первой строкой popup выбора героя. Ручной выбор применяется и на нечитаемом кадре (иначе слот, чей регион вообще не читается, остался бы в livepicks с прежним героем) |
...
| `g_gameInfo`, `g_portraitState`, `g_pickerState`, `g_spectatorState` | Экземпляры общих типов из shared_types.h. `g_spectatorState` - наблюдаемая (чужая) игра: пишет GSI-поток напрямую (livestatsfetcher.cpp), читает gui_draw.cpp (DrawDraftPanel), сбрасывает watchdog в orchestrator.cpp |
...
| `PumpMessages()` | Прокачка очереди сообщений без блокировки (между шагами обновления и внутри Set*). WM_QUIT из очереди не выбрасывается, а поднимает флаг отказа |
| `UpdateWindowAborted()` | true, если пользователь закрыл окно апдейтера крестиком. `UpdateWndProc` обрабатывает WM_DESTROY: при закрытии не нами (`DestroyUpdateWindow` помечает свой снос флагом) поднимает флаг и шлёт `PostQuitMessage`, а `RunStartupUpdateCheck` (mainGUI.cpp) проверяет флаг после запроса манифеста и перед возвратом. Без этого закрытый крестиком апдейтер оставлял процесс жить невидимым: GUI ещё не создан, `WaitForRetryClick` блокируется в `GetMessage` навсегда, и такой экземпляр держит exe залоченным (сборка не может обновить копию в корне проекта) |

---

### orchestrator.h / orchestrator.cpp
Фоновый оркестратор: управление GSI/portrait/picker потоками, livepicks/player_info, запуск фазы 1b. Вынесен из mainGUI.cpp (была `orchestratorMain`, ~315 строк - самая длинная функция в проекте, без единого top-level try/catch). Тред-менеджмент (`g_pickerRunning`/`g_portraitRunning`/потоки/`g_draftRefreshNeeded`/`g_orchestratorRunning`) - `static` внутри файла, наружу только функции.
...
| `requestDraftRefresh()` | Сигнал оркестратору: пользователь вручную поменял драфт в GUI - позицию слота или героя слота (вызывается из gui_draw.cpp вместо прямой записи в приватный атомик) |
| `writeManualOverridesToLivePicks(db)` | Запись ручных правок GUI в livepicks: позиции своей команды (`manualPos > 0`) + герои любого из 10 слотов, где `manualHero != 0` (>0 - heroId, -1 - очистка слота). Слоты без ручной правки не трогаются: после остановки капчура `detectedHero` уже не обновляется, и запись "того, что видел капчур" затёрла бы реальный драфт |
...
- One-shot: при ручной правке драфта в GUI (позиция или герой слота) вне фазы 3 -> запись в DB + запуск пикера до первого inferenceGen. Запись в livepicks идёт всегда, запуск пикера - только при accountId != 0 (без Friend ID пикера нет, но БД должна остаться согласованной с тем, что показано в GUI). Ожидание inferenceGen ограничено `ONESHOT_MAX_SEC` (10с), и `oneShotActive` дополнительно снимается в тир-дауне IDLE/POSTGAME: счётчик может не сдвинуться вообще (поток пикера снесли сразу после старта, несовместимая схема данных), а взведённый `oneShotActive` глушит отсчёт хвоста фазы 3 (`runPhaseStateMachine` выходит по раннему return) и блокирует все последующие one-shot'ы - захват и пикер жили бы до конца сессии
- Хвост фазы 3 (5с после конца HERO_SELECTION) останавливает потоки, но `g_portraitState` только помечает `active=false`, без `clear()`: матч продолжается, и `manualHero`/`detectedHero` нужны GUI до его конца. Полный `clear()` - в ветке IDLE/POSTGAME и в `handleNewMatch`
...
| `SetOurPosition(newPos)` | Меняет НАШУ настоящую позицию в текущем драфте (своп с тиммейтом при конфликте, запись в `g_portraitState.manualPos` + `requestDraftRefresh()`) - единая точка изменения позиции для бейджей Picks/Meta и слотов Draft |
...
| `SetOwnGameManualPos(absSlot, pos)` | Колбэк позиции для слотов СВОЕЙ игры - пишет `g_portraitState.manualPos[absSlot]` + `requestDraftRefresh()` (прежнее поведение `DrawHeroSlot` до вынесения записи в параметр `setPos`) |
| `SetOwnGameManualHero(absSlot, heroId)` | Колбэк ручной смены героя для слотов СВОЕЙ игры - пишет `g_portraitState.manualHero[absSlot]` (>0 герой, 0 вернуться к авто-детекту, -1 слот вручную пуст) + `requestDraftRefresh()`. Слот в `g_pickerState` обновляется тут же, не дожидаясь круга через livepicks - через 250-500мс оттуда приедет то же значение (капчур в фазе пиков, one-shot оркестратора вне её) |
...
| `DrawHeroSlot(rowW, h, ..., setPos, draftHeroes=nullptr, setHero={})` | Слот героя с кликабельным popup позиции (1-5, свап) и кликабельной строкой слота (popup смены героя). `setPos(slot, pos)` - `std::function<void(int,int)>`, вызывается попапом вместо прежней жёстко зашитой записи в `g_portraitState` - хранилище позиций (своя игра vs наблюдаемая) выбирает вызывающая панель: `DrawDraftPanel` передаёт `SetOwnGameManualPos`, `DrawSpectatorDraftPanel` - `SetSpectatorManualPos`. `setHero(slot, heroId)` + `draftHeroes[10]` (герои всех слотов, чтобы блокировать дубли) включают ручную смену героя; `DrawSpectatorDraftPanel` их не передаёт - в чужом матче герои приходят из GSI, править нечего. InvisibleButton строки слота подаётся ДО кнопки позиционного бейджа и с `SetNextItemAllowOverlap()`, иначе бейдж (поданный позже, т.е. сверху) перестал бы ловить свои клики |
| `DrawHeroPickPopup(popupId, rowW, absSlot, draftHeroes, setHero)` | Popup ручной смены героя: `InputTextWithHint` + прокручиваемый `BeginChild` на 5 строк. Enter выбирает верхнего доступного кандидата. Первая строка (только при пустом фильтре) - последний результат капчура `detectedHero[absSlot]`: оформлена как обычная строка списка (портрет героя + имя, либо "?" без имени, если не распознано ничего - без текстовой пометки вроде "detected"). Её выбор снимает ручную правку (0), а если распознано пусто и капчур уже не работает - помечает слот пустым (-1), т.к. очистить его больше некому. Герои, стоящие в других слотах, - `ImGuiSelectableFlags_Disabled` + тултип "Already in draft" |
| `loadHeroesIfNeeded()` / `heroSearchScore(q, entry)` / `heroSearchCandidates(filter)` | Ленивый справочник героев (один `SELECT id, name, localized_name FROM heroes` наполняет и `g_heroDisplayNames`, и `g_heroCatalog` с ключами `canonicalHeroKey`) + ранжирование кандидатов popup'а. Тиры: точное совпадение (1000) -> префикс (900-) -> аббревиатура по первым буквам слов, "pa"/"cm"/"sf" (850) -> подстрока (700-) -> подпоследовательность (600-) -> опечатка, Левенштейн <=2 (500-). Сравнение идёт и с `localized_name`, и с внутренним `heroes.name` - берётся лучший результат. Пустой фильтр - весь справочник по алфавиту |
...
| `heroDisplayName(heroId, fallback)` | heroId -> localized_name для отображения, ленивая загрузка через `loadHeroesIfNeeded()` (`SqliteDB(readOnly=true)`) |
...
Все кликабельные элементы (позиционные бейджи/теги, строка слота героя в своей игре, строки popup'а выбора героя, [D]-логотип, Refresh, Set/x в карточке игрока, иконки Powered by, кнопка `[screenshot]`) имеют `ImGui::SetTooltip(...)` при наведении (`ImGui::IsItemHovered()`) - поясняющий текст того, что делает клик. Заблокированная строка popup'а требует `ImGuiHoveredFlags_AllowWhenDisabled`, иначе тултип "Already in draft" не показывался бы именно там, где он нужен. Не покрыты: нативные Win32-кнопки вне ImGui (например "Try again" в `update_window.cpp`, показывается до инициализации ImGui).
...
`[Dirs] Name: "{app}\screenshots"` - пустая папка создаётся при установке для кнопки `[screenshot]` (`DrawDraftPanel`, gui_draw.cpp): туда сохраняются 10 регионов героев + 10 регионов позиций + `fullscreen_regions.png` (полный кадр HUD с рамками всех 20 регионов) + `report.txt` при диагностике ошибки распознавания драфта (см. `portrait_runner.h/.cpp`, `captureDebugScreenshotWithReport`).

`gamestate_integration_dota2.cfg`: `"heartbeat" "5.0"` (было `30.0`) - Dota 2 шлёт GSI-обновление раз в 5с даже без изменений в состоянии игры. Важно: старое значение 30.0 было **больше** watchdog-таймаута оркестратора на момент введения этой настройки - во время затишья в игре (без смены фазы/heroId) обновления от Dota могли не приходить дольше таймаута, и watchdog мог ложно сбрасывать статус на IDLE, хотя GSI-соединение оставалось живым. При текущем GSI_WATCHDOG_SEC (60с, `readGameStateWithTimeoutWatchdog` в orchestrator.cpp) 5.0 с запасом исключает этот сценарий.
