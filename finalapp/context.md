...
| `SetOwnGameManualPos(absSlot, pos)` | Колбэк позиции для слотов СВОЕЙ игры - пишет `g_portraitState.manualPos[absSlot]` + `requestPositionRefresh()` (прежнее поведение `DrawHeroSlot` до вынесения записи в параметр `setPos`) |
