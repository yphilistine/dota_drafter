...
- **Панель Meta Heroes** - топ-10 героев по популярности (STRATZ, сумма матчей, без ML) с винрейтом как вторичной метрикой, отдельная колонка рядом с рекомендациями, то же оформление; бейдж позиции сверху кликабельный всегда (popup All/Position 1-5) - во время игры меняет настоящую позицию в драфте (как в панели рекомендаций), а без активной игры просто переключает превью меты по позициям, не трогая драфт. Когда герой уже выбран - вместо топ-10 показывает его собственную мету на этой позиции (games/winRate), аналогично панели рекомендаций
...
- **Распознавание героев и позиций** - захват портретов каждые 250мс
...
- **Режим наблюдения (спектейт)** - если пользователь наблюдает чужой матч, а не играет сам, панель драфта показывает героев обеих команд напрямую из GSI (без захвата портретов - в HUD наблюдателя другая раскладка) с меткой "SPECTATING"; позиции только вручную (popup доступен для обеих команд). Win probability bar считается той же CatBoost-моделью, что и для своей игры (командный Component A), но без персональной поправки Component B - для чужого матча нет своего аккаунта. Панели рекомендаций и Meta Heroes в это время остаются в обычном состоянии ожидания - им личная статистика (accountId) действительно нужна, а наблюдаемому драфту нет
- **Кнопка Refresh** - перезагрузка данных игрока без перезапуска
- **Подсказки при наведении** - все кликабельные элементы интерфейса (позиции, [D]-логотип, Refresh, Set/x карточки игрока, иконки Powered by, [screenshot]) показывают поясняющий тултип
- **Кнопка [screenshot]** - рядом с заголовком панели драфта; сохраняет 10 регионов героев + 10 регионов позиций текущего кадра HUD, полноэкранный `fullscreen_regions.png` (весь кадр HUD с наложенными рамками всех 20 регионов героев/позиций) и `report.txt` (распознанное имя/score героя на слот, сырой OCR-текст/позиция/score на позиционный слот) в папку `screenshots\` - для диагностики, если распознавание драфта ошибается
...
- **GSI-таймаут** - автоматический сброс статуса при закрытии Dota 2 (60с без обновлений); GSI-конфиг шлёт heartbeat раз в 5с, чтобы это не срабатывало ложно во время затишья в игре
...
gamestate_integration_dota2.cfg     Конфигурация GSI (устанавливается инсталлятором, heartbeat 5.0)
screenshots/                        Регионы героев/позиций + fullscreen_regions.png + report.txt по кнопке [screenshot] (папка создаётся инсталлятором)
```
...
| `portrait_runner.cpp / .h` | Захват портретов + позиций (250мс цикл) + диагностический снимок для кнопки [screenshot] (`captureDebugScreenshotWithReport`) |
...
| Portrait capture (250мс) | ~2-6% (кратковременно) | ~5MB | - |
