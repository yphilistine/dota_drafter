...
- **Ручная смена героя в слоте** - клик по строке слота открывает popup: поиск по имени (лексически ближайшие по английскому и внутреннему имени - "pa", "shadow f", опечатки) либо выбор мышью из списка (5 строк видно, прокрутка). Работает во всех фазах и для обеих команд; ручной выбор имеет приоритет над распознаванием портретов, пока его не снимут. Первая строка списка показывает, что для этого слота в последний раз распознал захват экрана (портрет героя, либо "?", если ничего) - это же и возврат к автораспознаванию. Герои, уже стоящие в других слотах, недоступны
