...
- **Режим наблюдения (спектейт)** - если пользователь наблюдает чужой матч, а не играет сам, панель драфта показывает героев обеих команд напрямую из GSI (без захвата портретов - в HUD наблюдателя другая раскладка) с меткой "SPECTATING"; позиции только вручную (popup доступен для обеих команд). Панели рекомендаций и Meta Heroes в это время остаются в обычном состоянии ожидания - без личной статистики и ML-предсказаний, т.к. это не наш драфт
