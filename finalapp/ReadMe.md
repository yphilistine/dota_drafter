...
- **Панель Meta Heroes** — топ-10 героев по популярности (STRATZ, сумма матчей, без ML) с винрейтом как вторичной метрикой, отдельная колонка рядом с рекомендациями, то же оформление; позиция по умолчанию авто (следует за позицией игрока, иначе агрегат по всем позициям), но бейдж позиции сверху кликабельный — popup с Auto/All/Position 1-5 позволяет закрепить позицию вручную. Когда герой уже выбран — вместо топ-10 показывает его собственную мету на этой позиции (games/winRate), аналогично панели рекомендаций
