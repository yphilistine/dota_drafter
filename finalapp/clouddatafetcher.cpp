...
// --- RAII-обёртка PQconnectdb / PQfinish -------------------------------------
class PgConnection {
    PGconn* conn_;
public:
...
// --- immortalherostats (агрегат recentimmortalmatches) ------------------------
    ...
    const std::map<std::pair<int,int>, std::pair<int,int>>& agg)
{
    ...
    for (const auto& [key, val] : agg) {
        sqlite3_bind_int(stmt, 1, key.first);   // hero_id
        sqlite3_bind_int(stmt, 2, key.second);  // pos
        sqlite3_bind_int(stmt, 3, val.first);   // games
        sqlite3_bind_int(stmt, 4, val.second);  // wins
        rc = sqlite3_step(stmt);
        if (rc != SQLITE_DONE) {
            std::string e = sqlite3_errmsg(db);
            sqlite3_finalize(stmt);
...
void fetchAndStoreImmortalHeroStats(sqlite3* db, const std::string& connStr) {
    ...
    // Читаем все матчи одним запросом
    ...
    // Агрегируем: (hero_id, pos) -> (games, wins)
